            ...
            success: function (data) {
                ...
                if (data != null) {
                    for (var i = 0; i < data.length; i++) {
                        htmls += "<option value='" + data[i].id + "'>" + data[i].proName + "</option>";
                    }
                }
                $("#pdList").html(htmls);
            },
...
$(function () {
    $(".viewBill").on("click", function () {
        ...
        var obj = $(this);
        window.location.href = path + "/findBillByid?id=" + obj.attr("billid");
    });

    $(".modifyBill").on("click", function () {
        var obj = $(this);
        window.location.href = path + "/updateAndFindBillByid?id=" + obj.attr("billid");
    });
    $('#no').click(function () {
        cancleBtn();
    });
